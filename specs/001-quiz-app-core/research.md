...
**Decision**: A quiz can move `DRAFT` → `PUBLISHED` and back `PUBLISHED` → `DRAFT` (unpublish), but unpublish is allowed **only while the quiz has no attempts**. Once the first attempt exists (the same "locked" condition as FR-024), the quiz stays `PUBLISHED`; only its open/close dates remain editable.
**Rationale**: A teacher who publishes too early or spots a mistake needs a real way to pull the quiz back before any student has engaged with it, rather than the awkward workaround of pushing dates into the future. Gating unpublish on zero attempts reuses the existing lock rule, so it adds no new "is this editable" concept — and it keeps the integrity guarantee intact: once a student has started, the quiz's visibility can't be yanked out from under them.
**Alternatives considered**: One-directional publish (the earlier decision, now revised) — rejected because it left "I published too early" unsolved except by date-fudging. Allowing unpublish even after attempts exist — rejected: it would let a teacher hide a quiz a student has already begun or completed, undermining the same stability FR-024's lock protects.
